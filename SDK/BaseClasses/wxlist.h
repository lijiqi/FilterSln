...
#define __WXLIST__
   ...
   /* A POSITIONEX represents (in some fashion that's opaque) a cursor
      ...
      The POSITIONEX of an element in the list is only invalidated if
      that element is deleted.  Move operations may mean that what
      was a valid POSITIONEX in one list is now a valid POSITIONEX in
      a different list.
      ...
      This definition is the same as the POSITIONEX type for MFCs, so we must
      avoid defining it twice.
   */
#ifndef __AFX_H__

#ifdef DXSUB
struct __POSITIONEX {int unused;};
typedef __POSITIONEX* POSITIONEX;
#else
struct __POSITION { int unused; };
typedef __POSITION* POSITION;
typedef __POSITION* POSITIONEX;
#endif
...
public:
    ...
    __out_opt POSITIONEX GetHeadPositionI() const;


    /* Return a cursor which identifies the last element of *this */
    __out_opt POSITIONEX GetTailPositionI() const;
...
protected:
    ...
    */
    __out void *GetNextI(__inout POSITIONEX& rp) const;


    /* Return a pointer to the object at p
       Asking for the object at NULL will return NULL harmlessly.
    */
    __out_opt void *GetI(__in_opt POSITIONEX p) const;
    __out void *GetValidI(__in POSITIONEX p) const;
...
public:
    /* return the next / prev position in *this
       ...
       2. If you are keeping a list sorted (fairly common) and you
          ...
          to insert.  You then want that NULL to be a valid POSITIONEX
          so that you can insert before it and you want that insertion
          point to mean the (n+1)-th one that doesn't have a POSITIONEX.
    ...
    */
    __out_opt POSITIONEX Next(__in_opt POSITIONEX pos) const
    {
        if (pos == NULL) {
            return (POSITIONEX) m_pFirst;
        }
        CNode *pn = (CNode *) pos;
        return (POSITIONEX) pn->Next();
    } //Next

    // See Next
    __out_opt POSITIONEX Prev(__in_opt POSITIONEX pos) const
    {
        if (pos == NULL) {
            return (POSITIONEX) m_pLast;
        }
        CNode *pn = (CNode *) pos;
        return (POSITIONEX) pn->Prev();
    } //Prev
    ...
    */
protected:
    __out_opt POSITIONEX FindI( __in void * pObj) const;

    // ??? Should there be (or even should there be only)
    // ??? POSITIONEX FindNextAfter(void * pObj, POSITIONEX p)
    ...
    */
    __out_opt void *RemoveI(__in_opt POSITIONEX p);
    ...
    */
    __out_opt POSITIONEX AddTailI(__in void * pObj);
public:
    ...
    */
protected:
    __out_opt POSITIONEX AddHeadI(__in void * pObj);
public:
    ...
    */
protected:
    __out_opt POSITIONEX AddAfterI(__in_opt POSITIONEX p, __in void * pObj);
public:
    ...
    /* Add the list *pList to *this after position p in *this
       ...
       Existing POSITIONs in *this are undisturbed, including p.
    */
    BOOL AddAfter(__in_opt POSITIONEX p, __in CBaseList *pList);
    ...
    */
    protected:
    __out_opt POSITIONEX AddBeforeI(__in_opt POSITIONEX p, __in void * pObj);
    public:
    ...
    /* Add the list *pList to *this before position p in *this
       ...
       Existing POSITIONs in *this are undisturbed, including p.
    */
    BOOL AddBefore(__in_opt POSITIONEX p, __in CBaseList *pList);
    ...
    */
    ...
    /* The following operations do not copy any elements.
       ...
       positions muddled up - e.g. use a POSITIONEX that's in a different
    ...
    */
    BOOL MoveToTail(__in_opt POSITIONEX pos, __in CBaseList *pList);
    ...
    */
    BOOL MoveToHead(__in_opt POSITIONEX pos, __in CBaseList *pList);
...
public:
    ...
    __out_opt POSITIONEX GetHeadPosition() const { return (POSITIONEX)m_pFirst; }
    __out_opt POSITIONEX GetTailPosition() const { return (POSITIONEX)m_pLast; }
    int GetCount() const { return m_Count; }

    __out OBJECT *GetNext(__inout POSITIONEX& rp) const { return (OBJECT *) GetNextI(rp); }

    __out_opt OBJECT *Get(__in_opt POSITIONEX p) const { return (OBJECT *) GetI(p); }
    __out OBJECT *GetValid(__in POSITIONEX p) const { return (OBJECT *) GetValidI(p); }
    ...
    __out_opt OBJECT *Remove(__in_opt POSITIONEX p) { return (OBJECT *) RemoveI(p); }
    __out_opt POSITIONEX AddBefore(__in_opt POSITIONEX p, __in OBJECT * pObj) { return AddBeforeI(p, pObj); }
    __out_opt POSITIONEX AddAfter(__in_opt POSITIONEX p, __in OBJECT * pObj)  { return AddAfterI(p, pObj); }
    __out_opt POSITIONEX AddHead(__in OBJECT * pObj) { return AddHeadI(pObj); }
    __out_opt POSITIONEX AddTail(__in OBJECT * pObj)  { return AddTailI(pObj); }
    ...
    BOOL AddAfter(__in_opt POSITIONEX p, __in CGenericList<OBJECT> *pList)
            { return CBaseList::AddAfter(p, (CBaseList *) pList); };
    BOOL AddBefore(__in_opt POSITIONEX p, __in CGenericList<OBJECT> *pList)
            { return CBaseList::AddBefore(p, (CBaseList *) pList); };
    __out_opt POSITIONEX Find( __in OBJECT * pObj) const { return FindI(pObj); }
}; // end of class declaration
